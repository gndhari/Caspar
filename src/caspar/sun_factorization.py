...
def su2_parameters(U):
    ...
    if U.shape != (2, 2):
        ...
        return
    if not anp.isclose(anp.linalg.det(U), 1):
    ...
    b = None
    if anp.isclose(anp.absolute(U[0, 1]), 1):
        b = 2 * anp.arcsin(1)
    else:
        b = 2 * anp.arcsin(anp.absolute(U[0, 1]))

    arg_pos = anp.angle(U[0, 0]) #(a + g)/2
    arg_neg = -anp.angle(U[1, 0]) #(a - g)/2
...
def su3_parameters(U):
    ...
    elif anp.isclose(anp.abs(x), 1):
        # Compute the required phase matrix and embed into SU(3)
        phase_su2 = anp.matrix([[anp.conj(x), 0], [0, x]])

        full_phase_su2 = anp.asmatrix(anp.identity(3)) + 0j
...
def build_staircase(U):
    ...
    moduli = [anp.abs(U[x, 0]) for x in range(n)]
    if anp.allclose(sorted(moduli), [0.] * (n - 1) + [1]):
        ...
        if anp.isclose(running_prod[0, 0], 1):
        ...
        # to it). Now we need to add a separate phase shift as well.
            ...
            for rot_idx in range(n - 1, 0, -1):
                if running_prod[rot_idx, 0] != 0:
                    permmat = anp.matrix([[0, -1], [1, 0]])

                    full_permmat = anp.asmatrix(anp.identity(n)) + 0j
                    ...
                    if rot_idx == 1: # If we're at the top, add the phase too
                        phase_su2 = anp.matrix([[anp.conj(temp_product[0, 0]), 0], [0, temp_product[0, 0]]])
                        permmat = phase_su2 * permmat

                    transformations.append(su2_parameters(permmat.getH()))

                    full_trans = anp.asmatrix(anp.identity(n)) + 0j
        ...
        for rot_idx in range(n - 1):
            ...
            Rij_inv = anp.asmatrix(anp.identity(2)) + 0j
            full_Rij_inv = anp.asmatrix(anp.identity(n)) + 0j
            ...
            if rot_idx != n - 2:
                ...
                sum_of_column = 0
                for k in range(i):
                    sum_of_column += pow(anp.absolute(running_prod[k,0]), 2)
                cf = anp.sqrt(1 - sum_of_column)

                y, z = running_prod[i, 0], running_prod[j, 0]
                capY, capZ  = y / cf, z / cf

                # Build the SU(2) transformation and embed it into the larger matrix
                Rij_inv = anp.matrix([[anp.conj(capY), anp.conj(capZ)],
                                     [-capZ, capY]])
            else:
                ...
                x = U[0,0]
                cf = anp.sqrt(1 - pow(anp.absolute(x), 2))
                Rij_inv = anp.matrix([[anp.conj(x), cf],
                                     [-cf, x]])
...
def sun_factorization(U):
    """ Decompose an arbitrary element in SU(n) as a sequence of
        ...
        Takes as ianput a numpy matrix U.
    ...
    if not anp.allclose(U * U.getH(), anp.eye(n)):
